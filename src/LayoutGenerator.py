...
def draw_wire_row(cell, layer, cd, min_length, max_length, min_t2t, max_t2t, t2t_grid, max_x, location=np.array([0, 0]), enclosure=0):
    ...
    while total_x < max_x:
        ...
        if enclosure != 0:
            ast_wire_ll = pya.Point(wire_left+offset_x+enclosure, wire_lower+offset_y)
            ast_wire_ur = pya.Point(wire_right+offset_x-enclosure, wire_upper+offset_y)
...
def draw_wire_cell(spec):
    ...
    if thread>1:
        ...
        args_row_generator = zip([cell]*row_number, [l_m2]*row_number, [wire_cd]*row_number,
                                ...
                                [min_t2t]*row_number, [max_t2t]*row_number, [t2t_grid]*row_number,
...
class hilbert:
    ...
    def draw_init(self):
        ...
        self.cell.shapes(self.layer).insert(pya.Path([p1,p2,p3,p4], self.cd, self.cd/2, self.cd/2).polygon())
	
#        self.cell.shapes(self.layout.layer(10,0)).insert(pya.Box(self.cd,0,self.cd*2,self.cd))
        self.cell.shapes(self.layout.layer(self.via_layer,0)).insert(pya.Box(0,0,self.cd*2,self.cd))
	
	
    def next_level(self):
        self.level+=1
        print("Drawing Hilbert Level %g"%self.level)
        newcell=self.layout.create_cell(self.cell_prefix + "level_"+str(self.level))
        transll=pya.Trans(0, 0)
        translr=pya.Trans(self.box_size+self.length, 0)
        transul=pya.Trans(1, False, self.box_size+self.cd, self.box_size+self.length)
        transur=pya.Trans(3, False, self.box_size+self.length, self.box_size*2+self.length+self.cd)
        print("    Placing Cells...")
        ...
        p12=pya.Point(self.box_size+self.cd/2+self.length, self.box_size+self.cd/2)
        p21=pya.Point(self.cd/2, self.cd/2+self.box_size)
        p22=pya.Point(self.cd/2, self.cd/2+self.box_size+self.length)
        p31=pya.Point(self.box_size*2+self.length+self.cd/2, self.cd/2+self.box_size)
        p32=pya.Point(self.box_size*2+self.length+self.cd/2, self.cd/2+self.box_size+self.length)
...
class peano:
    ...
    def draw_init(self, type):
        ...
        if type==1:
            self.cell.shapes(self.layer).insert(pya.Path([p1,p5,p31,p41,p2,p6], self.cd, self.cd/2, self.cd/2).polygon())
        elif type==2:
            self.cell.shapes(self.layer).insert(pya.Path([p1,p2,p32,p42,p5,p6], self.cd, self.cd/2, self.cd/2).polygon())
	...
	#translation class transforms based on Rotation, Mirror, X, Y -- 4 item list
        ...
        trans12=pya.Trans(0, True, self.length + self.box_size, self.box_size+self.length/2)
...
#        trans=[trans11, trans12, trans13, trans21, trans22, trans23, trans31, trans32, trans33]
        ...
        paths.append(pya.Path([p1,p2], self.cd, self.cd/2, self.cd/2).polygon())

        p1=pya.Point(self.cd/2+self.box_size, self.cd/2+self.box_size)
        p2=pya.Point(self.cd/2+self.box_size, self.cd/2+self.box_size+self.length)
        paths.append(pya.Path([p1,p2], self.cd, self.cd/2, self.cd/2).polygon())

        p1=pya.Point(self.cd/2+self.box_size+self.length, self.cd/2+self.box_size)
        p2=pya.Point(self.cd/2+self.box_size+self.length, self.cd/2+self.box_size+self.length)
        paths.append(pya.Path([p1,p2], self.cd, self.cd/2, self.cd/2).polygon())   

        p1=pya.Point(self.cd/2+2*(self.box_size+self.length), self.cd/2+self.box_size)
        p2=pya.Point(self.cd/2+2*(self.box_size+self.length), self.cd/2+self.box_size+self.length)
        paths.append(pya.Path([p1,p2], self.cd, self.cd/2, self.cd/2).polygon())   
